...
#A6: joint 95 ci
library(ellipse)
plot(ellipse(sat_model, c('ratio', 'salary')), 
     type = "l")
points(0,0, pch=1)
...
plot(gamb_model$fitted, gamb_model$residuals,
...
#B2: normality
qqnorm(gamb_model_s$residuals, ylab = "Residuals")
qqline(gamb_model_s$residuals)
shapiro.test(gamb_model_s$residuals)

#B3: leverage points
halfnorm(lm.influence(gamb_model_s)$hat, nlab = 4, ylab = "Leverages")
